...
func Handle(w http.ResponseWriter, r *http.Request) {
	...
	var req authorization.RevokeRightsRequest
	...
	resp, err := dal.RevokeRights(&req)
	if err != nil {
		Core.Logger.Log(logger.ERROR, "failed to revoke rights for caller: "+caller+", error: "+err.Error())
		http.Error(w, "internal server error", http.StatusInternalServerError)
		return
	}

	if !resp.Valid {
		Core.Logger.Log(logger.WARN, "RevokeRights operation was not valid for caller: "+caller+", error: "+resp.Error)
	...
	Core.Logger.Log(logger.DEBUG, "Successfully revoked rights for entity: "+req.Entity.String()+" for caller: "+caller)
}
